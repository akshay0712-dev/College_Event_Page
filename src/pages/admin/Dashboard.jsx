...
const Dashboard = () => {
  ...
  // Registrations State + Stats + Filter
  ...
  useEffect(() => {
    ...
    unsubscribers.push(
      onSnapshot(query(collection(db, 'events'), orderBy('order', 'asc')), (snapshot) => {
        setTimelineEvents(snapshot.docs.map((d) => ({ id: d.id, ...d.data() })));
      })
    );

    unsubscribers.push(
      onSnapshot(query(collection(db, 'videos'), orderBy('order', 'asc')), (snapshot) => {
        setVideos(snapshot.docs.map((d) => ({ id: d.id, ...d.data() })));
      })
    );

    unsubscribers.push(
      onSnapshot(query(collection(db, 'speakers'), orderBy('order', 'asc')), (snapshot) => {
        setSpeakers(snapshot.docs.map((d) => ({ id: d.id, ...d.data() })));
      })
    );

    unsubscribers.push(
      onSnapshot(query(collection(db, 'team'), orderBy('order', 'asc')), (snapshot) => {
    ...
    unsubscribers.push(
      onSnapshot(query(collection(db, 'sponsors'), orderBy('tier', 'asc')), (snapshot) => {
  ...
  useEffect(() => {
    const collections = [
      'tickets', 'news', 'events', 'speakers', 'team',
      'gallery', 'videos', 'sponsors', 'registrations'
    ];
  ...
  // ── Registration Handlers with Email ──
  const handleVerifyRegistration = async (regId, status, message = '') => {
    ...
    try {
      ...
      try {
        ...
        alert(
          `Registration ${status === 'verified' ? 'Verified ✅' : 'Rejected ❌'} but email failed.`
        );
      }
  ...
  const handleBulkVerify = async () => {
    ...
    if (
      !window.confirm(
        `Verify all ${pendingRegs.length} pending registrations?\nThis will send welcome emails to all of them.`
      )
    )
      return;
  ...
  const handleAddTicket = async (e) => {
    ...
    setLoading(true);
    try {
      const featuresArray = ticket.features
        .split(',')
        .map((f) => f.trim())
        .filter(Boolean);
      const data = {
    ...
    <div className="md:col-span-2">
      ...
      >
        ...
        {upcomingEvents.map((ev) => (
          <option key={ev.id} value={ev.id}>⏭️ {ev.title}</option>
        ))}
          ...
          <div className="max-w-6xl mx-auto">
            ...
            <header className="flex justify-between items-center mb-8 border-b border-white/10 pb-6">
              <div>
                ...
                <p className="text-gray-500 text-sm mt-1">
                  GEC Kishanganj • Full CRUD • Email Notifications
                </p>
              </div>
              ...
              <button
                ...
                onClick={async () => {
                  try {
                    ...
                    alert('Migration utility not found. This is optional.');
                  }
                }}
            ...
            <div className="space-y-6">
                ...
                <>
                  ...
                  <div className="bg-white/5 border border-white/10 rounded-3xl p-8">
                    <h3 className="text-xl font-bold mb-4">
                      ...
                      <span className="ml-3 text-sm text-amber-500 font-normal">
                    ...
                    <div className="space-y-4">
                            ...
                            <div className="flex-1">
                              ...
                              <div className="grid grid-cols-2 gap-3 text-sm mb-4">
                                <div>
                                  ...
                                  <span className="text-gray-200">{reg.email}</span>
                                </div>
                                <div>
                                  <span className="text-gray-500">Phone:</span>{' '}
                                  <span className="text-gray-200">{reg.phone}</span>
                                </div>
                                <div>
                                  <span className="text-gray-500">Reg No:</span>{' '}
                                  <span className="text-gray-200">{reg.registrationNo}</span>
                                </div>
                                <div>
                                  ...
                                  <>
                                    <button
                                      onClick={() => {
                                        ...
                                        handleVerifyRegistration(
                                          reg.id,
                                          'verified',
                                          m || 'Payment verified! Welcome to Fresher 2026.'
                                        );
                                      }}
                                      disabled={loading}
                                    ...
                                    <button
                                      onClick={() => {
                                        ...
                                        if (m)
                                          handleVerifyRegistration(reg.id, 'rejected', m);
                                      }}
                                      disabled={loading}
                      ...
                      {filteredRegistrations.length === 0 && (
                        <p className="text-center text-gray-500 py-12">
                          No {filterStatus === 'all' ? '' : filterStatus} registrations yet
                        </p>
                      )}
                    </div>
                  </div>
              ...
              {selectedRegistration && (
                ...
                >
                  <div
                    className="bg-[#111] border border-white/20 rounded-2xl p-6 max-w-2xl w-full"
                    onClick={(e) => e.stopPropagation()}
                  >
                    <div className="flex justify-between items-center mb-4">
                      <h3 className="text-xl font-bold">
                        Payment — {selectedRegistration.fullName}
                      </h3>
                      <button
                        onClick={() => setSelectedRegistration(null)}
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                          ...
                          <input type="text" placeholder="GEC Fresher Party 2026" value={heroEvent.title}
                            ...
                            className="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-3 text-white outline-none focus:border-amber-500" required />
                        </div>
                        <div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div className="md:col-span-2">
                          ...
                          <textarea placeholder="Entry, Welcome Drink, VIP Seating" value={ticket.features}
                            ...
                            rows="3" className="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-3 text-white outline-none focus:border-amber-500" required />
                        </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div className="md:col-span-2">
                          ...
                          <textarea placeholder="Brief description..." value={videoHighlight.description}
                            ...
                            rows="2" className="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-3 text-white outline-none focus:border-amber-500" />
                        </div>
                        <div>
                    ...
                    <p className="text-xs text-gray-500 mb-2">
                      ...
                      {linkedItemsCounts[ev.id] && (
                        <>
                          <br />
                          📎{' '}
                          {Object.values(linkedItemsCounts[ev.id]).reduce((s, c) => s + c, 0)}{' '}
                          items
                        </>
                      )}
                    </p>
